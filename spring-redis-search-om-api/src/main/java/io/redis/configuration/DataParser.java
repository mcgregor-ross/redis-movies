...
public class DataParser implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void readData() throws IOException {
        ...
        // Get all JSON Files
        JsonReader reader = new JsonReader(new FileReader( "src/main/resources/parsed-movies.json"));
        ...
        while (movieIT.hasNext()) {
            log.info("Processing json movie number: {} of {}", processed++, moviesJSON.size());
            JsonObject movie = movieIT.next().getAsJsonObject();

            String releaseDate = movie.get("release_date").getAsString();
            List<String> cast = this.getCastCrewNames(movie.getAsJsonArray("cast"));
            List<String> directors = this.getCastCrewNames(movie.getAsJsonArray("director"));
            List<String> writers = this.getCastCrewNames(movie.getAsJsonArray("writer"));

            long releaseDateUTC;
            if (StringUtils.isNotBlank(releaseDate)) {
                releaseDateUTC = LocalDate.parse(releaseDate).atStartOfDay().toEpochSecond(ZoneOffset.UTC);
                movie.add("releaseDateUTC", gson.toJsonTree(releaseDateUTC));
            }

            movie.add("actors", gson.toJsonTree(cast));
            movie.add("directors", gson.toJsonTree(directors));
            movie.add("writers", gson.toJsonTree(writers));

            restructuredJSON.add(movie);
        }

        this.writeToFile(restructuredJSON, "parsed-movies-temp.json");

    }

    List<String> getCastCrewNames(JsonArray jsonArray) {
        List<String> cast = new ArrayList<>();
        Iterator<JsonElement> castIT = jsonArray.iterator();
        while (castIT.hasNext()) {
            JsonObject jsonObject = castIT.next().getAsJsonObject();
            cast.add(jsonObject.get("name").getAsString());
        }
        return cast;
    }
